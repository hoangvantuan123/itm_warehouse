...
export default function ModalHelpMenu({ openHelp, setOpenHelp, fetchDataSearchRootMenu, setOnSelectRow, inputHelp }) {
    const [searchResults1, SetSearchResults1] = useState([])
    const [error1, setError1] = useState(null)
    const [loading1, setLoading1] = useState(false)

    const getSearchResults = async (searchText) => {
        const results = await fetchDataSearchRootMenu(searchText);
        SetSearchResults1(results);
    };

    useEffect(() => {
        if (openHelp && inputHelp !== null) getSearchResults(inputHelp)
    }, [openHelp, inputHelp])
