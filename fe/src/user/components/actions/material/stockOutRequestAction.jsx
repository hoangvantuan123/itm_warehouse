...
export default function StockOutRequestAction() {
    ...
    const btnPrint = async () => {
        try {
            ...
            const res = await fetch("http://localhost:8080/api/v1/print/test?matID=DIO0032&lotNo=RU338144G01")
            ...
            reader.onload = (e) => {
                ...
                const convert = function replaceBase64WithImgTag(htmlContent) {
                    const regex = />\s*(data:image\/(?:png|jpeg|jpg|gif);base64,([A-Za-z0-9+/=]+))/g;
...
^XZ `;
        ...
        try {
            const response = await fetch('https://api.labelary.com/v1/printers/8dpmm/labels/4x6/0/', options);
